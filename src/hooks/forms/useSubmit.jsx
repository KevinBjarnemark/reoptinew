...
const useSubmit = (showDebugging = true) => {
     ...
     * and backend (API response) levels, ensuring debugging logs and user-friendly 
     * alerts are generated as needed.
     * 
     * Note, that formDataDraft fields containing the value null, will be skipped.
     *
     ...
     * @param {string} data.relativeURL The relative URL for the backend endpoint 
     ...
     * @param {string} [data.debugMessages.successfulBackEndResponse] Message to 
     * log when the back end response was successful.
     ...
     *     validateForm: () => {
     *         if (!formDataDraft.username) throw new Error("Username is required.");
     *         if (formDataDraft.password.length < 8) throw new Error("Password too short.");
     *     },
     *     formDataDraft: { username: "exampleUser", password: "securePassword" },
    ...
    const submitForm = async (data) => {
        ...
        try {
            ...
            */
            const errorMessage = error.message || "Validation failed. Please try again.";
