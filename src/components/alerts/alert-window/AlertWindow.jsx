...
const AlertType = ({messageIndex}) => {
    ...
    return (
        <h5 style={{opacity: alerts?.length > 0 ? 1 : 0}}>
            ...
            >
            </i> {`${alerts[messageIndex]?.alertType}`}
        </h5>
    );
}
...
const ToggleButtons = (props) => {
    ...
    if (alerts?.length > 0) {
        return (
            <div className="flex-row-absolute" style={{right: "50px", top: 0}}>
                <p 
                    className={`flex-row-absolute ${style["toggle-alert-page"]}`}
                    style={{color:"#ffffff"}}>
                ...
                <button
                    ...
                    onClick={() => {
                        setMessageIndex(prev => Math.min(prev + 1, alerts.length - 1));
                    }}>
                        ►
                </button>
            </div>
        ...
        <>
            ...
            {alerts?.length > 1 ? 
                <section 
                    ...
                    style={{
                        transform: `${transformTranslate} translate(3%, 8%) rotate(-7deg)`,
                        boxShadow,
                    }} 
                >
                    <div className={`flex-row-relative ${style["top-bar"]}`}>
            ...
            >
                ...
                <div className={`flex-row-relative w-100 ${style["content"]}`}>
                    { alerts ?
                        <p>{alerts[messageIndex]?.message}</p>
                    : null}
                </div>
            </section>
        </>
